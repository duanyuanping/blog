...
![15397817879101539781787910](http://p86utdi99.bkt.clouddn.com/1539781787910.png)
...
还记得前面我们请求登录中心系统提供的检测中心系统是否含有此用户的登录信息的接口时再请求头中写入的 origin 字段吗？这个接口实际完成的工作不仅仅只是判断用户是否登录，当我们请求他的时候，他会将请求头中的 origin 字段存放到用户登录信息中，这样就能够在后面对这些系统统一发起退出操作了。
...
async checkUser() {
  ...
  try {
    ...
    // 有这个用户的登录信息，就像此用户信息中的 logined 数组中添加 origin
    user.logined.push(origin);
