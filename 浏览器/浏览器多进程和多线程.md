...
js 还可以读取和修改 Style-Tree 属性，当 Style-Tree 构建时，js 执行将会暂停，直到 Style-Tree 构建完成。（？？？，实际操作中得出 js 在构建 Style-Tree 之前会执行，如果执行的 js 代码修改了页面元素的样式，那么此次修改就会放入 GUI 渲染等待队列中去。）
...
上面这段代码打印出来的是空字符串，也就是说在 js 执行的时候 CSS 还没有解析成 Style-Tree。还有一点就是不管 style 标签放在 script 标签前面还是后面，只要 js 修改了元素样式，就会以 js 修改的这个样式来展示。
